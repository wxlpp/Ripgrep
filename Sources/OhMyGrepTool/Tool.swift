...
extension OhMyGrep {
    ...
    {
      "name": "oh_my_grep",
      "description": "Search files with oh-my-grep (ripgrep engine, rg-compatible subset). Provide arguments as you would on the rg CLI; behavior matches rg defaults (case-sensitive, respects .gitignore). Examples:\n  \"TODO src/\"\n  \"-S 'func\\s+\\w+' src/ -t swift -A 2\"\n  \"-i error logs/ -g '*.log' -m 50\"\nUnsupported flags: --pre, -z, --type-add, --hyperlink-format, --sort modified, --vimgrep, --binary. Relative paths (and the default when no path is given) resolve against the host-provided working directory. Warnings (unreadable paths, binary files) follow the results as `path: message` lines. Use --json to get JSON-lines output: one match object per line, then one {\"warning\": {...}} object per warning.",
      "input_schema": {
        "type": "object",
        "required": ["args"],
        "properties": {
