...
>
> ### v0.3 perf backlog (recorded, not in scope for v0.1/v0.2)
> - **V3-perf-1:** `ChannelSink` clones `path: String` per match (ccd v0.2 Major #1). Real fix needs an FFI result-model redesign (`SearchMatch.path` is a UniFFI `Record String`, owned per-result by contract — `Arc<str>` on the sink can't remove the required owned `String`).
> - **V3-perf-2:** `pending_after` after-context attribution is O(pending_after) per After line within a context group (PR#2 #3). Bounded today; evict matured matches (`m_line + after_context < abs_line`) to keep it tight if it ever matters.
>
