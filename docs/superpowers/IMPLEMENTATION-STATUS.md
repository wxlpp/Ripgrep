...
> - **V3-perf-1 ✅ CLOSED — ACCEPT + DOCUMENT (maintainer decision)** (`<doc commit>`). The per-match owned `path: String` is inherent to the UniFFI `SearchMatch` Record contract (each result owns its path; `Arc<str>` on the sink can't remove it). The only real fix — an FFI result-model redesign (group-by-file) — is a breaking public-API change (Swift `Ripgrep.SearchResult/Match`, formatters, Tasks 22/23/25 tests, JSON tool-output shape, all consumers) not justified for a micro-allocation an IO/regex-bound, `max_matches`-truncated workload doesn't feel. Decision: keep as-is, documented in `sink.rs` at the `path: self.path.clone()` site so future reviewers (Copilot/ccd) don't re-raise it. Consistent with the project-wide YAGNI/cost-benefit dispositions (cf. P2/P3 refuted, P5/P6b YAGNI). **v0.3 perf backlog COMPLETE (V3-perf-2 fixed, V3-perf-1 accepted+documented).**
>
> **MERGE-前 ADVERSARIAL GATE (ccd, 2026-05-17, substitute for `/codex:adversarial-review`):** ran on the v0.3-perf diff (3 commits vs origin/main `7b9348a`: `sink.rs`+`lib.rs`+`pending_after_bound_tests.rs`). Verdict **0 Critical / 0 Major; zero issues in the code the 3 commits introduced** (`flush_matured`, the bound, the test, the V3-perf-1 accept-document comment). All Minor/Nit either reviewer-confirmed non-issues (#4 two-pass drain no impact, #6 false-positive, #7/#11 ack-no-change, #10 no bug) or doc nits (#9). The two "most actionable" items are **pre-existing & out of scope** — verified the v0.3 diff modifies zero `trim_end_matches`/`unwrap_or(0)`/`from_utf8_lossy` lines (identical to origin/main since v0.1.0): **#1 CRLF `\r` residue** (`from_utf8_lossy(...).trim_end_matches('\n')` leaves trailing `\r` on `\r\n` files → `line`/before/after strings) — genuine latent data-quality bug, cross-platform behavior change needing its own task+tests; **#2** `m.line_number().unwrap_or(0)` — add `debug_assert!` (already has explanatory inline comment + analogous `debug_assert!` in sibling `context()` else-branch). Per `receiving-code-review` (verify→scope→push back) + CLAUDE.md (out-of-flow ad-hoc changes don't trigger; finishing gate = adversarial review of *the branch's* changes), NOT scope-creeping the perf branch. **→ v0.4 BACKLOG (deferred, reasoned):** #1 CRLF-aware line trim (`trim_end_matches(['\r','\n'])` or strip trailing `\r` after `\n`) across `matched()`+`context()`, with CRLF-fixture tests; #2 promote the `unwrap_or(0)` comment to `debug_assert!`. Mirrors v0.2's #3→v0.3 deferral pattern.
>
> ### v0.4 — EXECUTION (branch `worktree-v0.4-crlf`, off merged-main `292765f`; PR #3 merged 2026-05-17)
> Scope = the two deferred v0.3-gate items, executed via subagent-driven-development (implementer + spec review + code-quality review per task).
> - **v0.4-CRLF-1 ✅ DONE** (`aad6320`) — extracted `fn decode_line(bytes:&[u8])->String` = `from_utf8_lossy` → `trim_end_matches('\n')` → `strip_suffix('\r')` (single `\r`, NOT `trim_end_matches('\r')` which would strip content `\r`/diverge from rg), called from both `matched()` & `context()` (anti-drift). New `src/tests/crlf_tests.rs` (5 tests, RAII-cleaned temp files, runtime-constructed CRLF bytes — NO committed fixture, git-EOL-safe). Non-vacuity proven by spec reviewer (revert `strip_suffix` → 41 pass/3 fail; restore → 44). Spec✅ + code-quality✅ ("merge: Yes"). LF/no-terminator/multiline-internal `\r\n` behavior-preserved.
> - **v0.4-CRLF-2 ✅ DONE** (`77e6804`) — `matched()` `m.line_number()` None case now `unwrap_or_else(|| { debug_assert!(false, "ChannelSink: m.line_number() is None; SearcherBuilder must enable line_number(true)"); 0 })` — byte-exact mirror of the sibling `context()` debug_assert; **release behavior identical** (debug_assert no-op → silent `0` as before). Diff-verified (+4/-1, sink.rs only).
> - **MERGE-前 ADVERSARIAL GATE (ccd, 2026-05-18):** ran on the v0.4 diff (`sink.rs`+`lib.rs`+`crlf_tests.rs` vs origin/main `292765f`). Verdict **0 Critical / 0 Major / 3 Minor / 2 Nit; "代码质量高"**. Disposition (`receiving-code-review`): **FIXED #3** temp-file leak → RAII `TempFile(PathBuf)` Drop guard, no new dep (in scope: the new test file; 2 reviewers flagged; my earlier "keep consistency" call was on a false premise — pre-existing tests use the committed fixture, not temp files); **ACCEPT+DOCUMENT #1/#4** multiline double-CRLF-terminator residual `\r` → one doc-comment line + correctness-table row (reviewer's *own* primary rec; loop-fix introduces content-`\r` ambiguity; pre-existing `trim_end_matches('\n')` greediness since v0.1.0, out of scope to "fix"); **PUSH BACK #5** `expect()`-in-release → keep `debug_assert!`+safe fallback (panicking in release would regress the project-wide "no public-API path crashes the host" invariant — PR#1 #4 / ccd Major#2 / PR#2 #2; None is unreachable since search.rs always sets `line_number(true)`; reviewer rates lowest priority); **DEFER #2** `context_break` lacks cancel poll → low-frequency by reviewer's own words, pre-existing & untouched by v0.4, v0.2-P6 already established the matched/context event/byte cadence as the cancel-latency lever — recorded as low-value v0.x backlog candidate, not scope-creeping a CRLF branch. Gate fixes applied in `06253dd` (sink.rs comment-only + crlf_tests RAII; delta-verified: no `context_break`/`expect()`/`decode_line`-logic change). **v0.4 COMPLETE** — cargo 44 (39+5) / swift 50 / clippy / fmt clean.
> - **v0.4 BRANCH FINISHED via push+PR — PR #4 opened** (https://github.com/wxlpp/Ripgrep/pull/4, base `main`, 4 commits + this doc, 2026-05-18). Worktree `.claude/worktrees/v0.4-crlf` preserved for PR-feedback iteration (finishing-a-development-branch Option 2). Mirrors v0.1.0→PR #1, v0.2→PR #2, v0.3→PR #3.
>
