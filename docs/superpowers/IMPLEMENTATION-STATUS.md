...
> - **P7** pending — extract lib.rs `#[cfg(test)]` modules into separate files (pure behavior-preserving cleanup; lowest priority/risk; must stay crate-internal `#[cfg(test)]` unit tests NOT `tests/` integration since they use internal items; mind Deviation #1 fixture paths & #3 cross-module `req()` helper visibility; identical test count/names/results required).
>
