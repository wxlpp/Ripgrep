...
> - **P6..P7** pending. P6 = CANCEL_CHECK_EVERY hybrid event/byte cadence (the REAL cancel-latency lever for giant single-line files, where P2's concern legitimately lands) + a clarifying comment on `Submatch` u32 (NO u64 widen — >4GB single line is non-realistic; YAGNI). P7 = extract lib.rs test modules.
>
