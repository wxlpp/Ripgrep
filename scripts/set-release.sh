...
# Switches Package.swift from the local XCFramework to the given release's zip.
set -euo pipefail
cd "$(dirname "$0")/.."
...
perl -0pi -e "s/^let useLocalBinary = true\$/let useLocalBinary = false/m; s/^let releaseVersion = \"[^\"]*\"\$/let releaseVersion = \"$VERSION\"/m; s/^let releaseChecksum = \"[^\"]*\"\$/let releaseChecksum = \"$CHECKSUM\"/m" Package.swift

grep -q "^let useLocalBinary = false$" Package.swift || { echo "ERROR: useLocalBinary not switched" >&2; exit 1; }
